...
def engine(database):
    ...
    with owner.begin() as db:
        db.execute(text("TRUNCATE analytics_confirmations,svc_dispositions,svc_charges,svc_costs,svc_rma_inspections,svc_rma_returns,svc_rma_lines,svc_rmas,svc_change_reversals,svc_old_parts,svc_changes,svc_spare_returns,svc_issues,svc_reservations,svc_tests,svc_returns,svc_receipts,svc_events,svc_works,fin_adjustment_corrections,fin_commands,fin_source_adjustments,fin_reversals,fin_invoice_lines,fin_invoices,fin_refunds,fin_allocations,fin_releases,fin_adjustments,fin_cash,fin_plans,del_reversals,del_returns,del_acceptances,del_lines,del_shipments,del_tests,asm_installations,asm_completions,asm_devices,asm_issue_lines,asm_issues,asm_events,asm_reservations,asm_requirements,asm_works,inv_amendments,inv_attachments,inv_import_rows,inv_imports,inv_opening_policy,inv_balances,inv_entries,inv_lots,inv_units,inv_layers,inv_movements,inv_receipt_lines,inv_receipts,inv_locations,inv_cancellations,inv_order_lines,inv_orders,inv_contract_lines,inv_contracts,inv_tracking,inv_commands,inv_suppliers,contract_commands,contract_editions,sales_orders,signed_files,signed_contracts,contract_files,contract_payments,contract_workspaces,publication_commands,contract_drafts,publication_events,quote_version_states,quote_versions,publication_decisions,publication_candidates,publication_policies,quote_commands, quote_exclusions, quote_selections, quote_drafts, jobs, outbox_events, crm_commands, crm_role_history, crm_responsibilities, crm_sites, crm_project_people, crm_projects, crm_contacts, crm_customers"))
    owner.dispose()
