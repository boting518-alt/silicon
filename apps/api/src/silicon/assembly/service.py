...
def complete(db,a,id,b,request_id):
    ...
    if db.scalar(text('SELECT 1 FROM inv_units WHERE sku_id=:s AND serial_normal=:sn'),{'s':w['product_sku_id'],'sn':sn}):raise Denied(409,'SERIAL_ALREADY_EXISTS')
    ...
    layer=inv.create_layer(db,a,m,inv.row(db,'catalog_skus',w['product_sku_id']),1,'sn',b.serial,'',b.location_id,'pending',cost)
    finished=inv.row(db,'inv_layers',layer);did=uuid4()
    inv.insert(db,a,'asm_devices',{'id':did,'work_id':id,'inventory_unit_id':finished['unit_id'],'layer_id':layer,'movement_id':m,'number':'DEV-'+str(did),'product':json.dumps(product)})
    for x in parts:
        req=next(r for r in w['requirements'] if r['sku_id']==x['sku_id'])
        inv.insert(db,a,'asm_installations',{'id':uuid4(),'device_id':did,'layer_id':x['layer_id'],'unit_id':x['unit_id'],'quantity':x['quantity'],'position':req['position']})
...
def reverse(db,a,id,b,request_id):
    ...
    device=db.execute(text('SELECT * FROM asm_devices WHERE work_id=:w AND movement_id=:m'),{'w':id,'m':b.movement_id}).mappings().first()
    ...
    if device:
        db.execute(text('UPDATE asm_installations SET removed_at=clock_timestamp(),correction_id=:r WHERE device_id=:d'),{'r':rid,'d':device['id']})
...
def device(db,id):
    ...
    d['cost']=str(inv.row(db,'inv_layers',d['layer_id'])['unit_cost']);d['cost_scope']='仅实际领料成本';d['delivery_state']='not_started'
...
def reconciliation(db,id):
    ...
    outputs=list(db.execute(text('SELECT d.*,l.unit_cost FROM asm_devices d JOIN inv_layers l ON l.id=d.layer_id AND l.tenant_id=d.tenant_id WHERE d.work_id=:id AND NOT EXISTS(SELECT 1 FROM inv_movements m WHERE m.reverse_of=d.movement_id)'),{'id':id}).mappings())
