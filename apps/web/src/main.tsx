...
const navigation=[['▧','业务驾驶舱'],['◇','报价与利润'],['✓','报价审批'],['▤','交付与验收'],['⚙','售后与部件'],['↗','续保与扩容'],['◫','经营总览'],['◉','客户管理'],['▦','商品目录'],['◇','包件 / BOM'],['▤','价格维护'],['▤','合同管理'],['▤','销售订单'],['⚙','装配工单'],['▥','设备查询'],['▣','采购合同'],['▦','库存管理'],['◷','周转分析']];
...
function App() {
  const [activePage,setActivePage]=useState<'crm'|'quotes'|'publication'|'contracts'|'orders'|'inventory'|'assembly'|'devices'|'delivery'|CatalogPage>('crm');
  const pages:Record<string,'crm'|'quotes'|'publication'|'contracts'|'orders'|'inventory'|'assembly'|'devices'|'delivery'|CatalogPage>={'交付与验收':'delivery','装配工单':'assembly','设备查询':'devices','采购合同':'inventory','库存管理':'inventory','销售订单':'orders','报价审批':'publication','合同管理':'contracts','报价与利润':'quotes','客户管理':'crm','商品目录':'skus','包件 / BOM':'boms','价格维护':'price-books'};
