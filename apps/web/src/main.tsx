...
function App() {
  const [activePage,setActivePage]=useState<'crm'|'quotes'|'publication'|'contracts'|'orders'|CatalogPage>('crm');
  const pages:Record<string,'crm'|'quotes'|'publication'|'contracts'|'orders'|CatalogPage>={'销售订单':'orders','报价审批':'publication','合同管理':'contracts','报价与利润':'quotes','客户管理':'crm','商品目录':'skus','包件 / BOM':'boms','价格维护':'price-books'};
