...
import {PublicationDesk} from './Publication';
...
const navigation=[['▧','业务驾驶舱'],['◇','报价与利润'],['✓','报价审批'],['▤','交付与验收'],['⚙','售后与部件'],['↗','续保与扩容'],['◫','资金工作台'],['◉','客户管理'],['▦','商品目录'],['◇','包件 / BOM'],['▤','价格维护'],['▤','合同管理'],['▤','销售订单'],['⚙','装配工单'],['▥','设备查询'],['▣','采购合同'],['▦','库存管理'],['◷','周转分析']];
...
function App() {
  const [analyticsTarget,setAnalyticsTarget]=useState<{kind:string;id:string}|null>(null);
  const [serviceDevice,setServiceDevice]=useState('');
  const [activePage,setActivePage]=useState<'crm'|'quotes'|'publication'|'contracts'|'orders'|'inventory'|'assembly'|'devices'|'delivery'|'finance'|'service'|'analytics'|CatalogPage>('analytics');
  const pages:Record<string,'crm'|'quotes'|'publication'|'contracts'|'orders'|'inventory'|'assembly'|'devices'|'delivery'|'finance'|'service'|'analytics'|CatalogPage>={'业务驾驶舱':'analytics','周转分析':'analytics','售后与部件':'service','资金工作台':'finance','交付与验收':'delivery','装配工单':'assembly','设备查询':'devices','采购合同':'inventory','库存管理':'inventory','销售订单':'orders','报价审批':'publication','合同管理':'contracts','报价与利润':'quotes','客户管理':'crm','商品目录':'skus','包件 / BOM':'boms','价格维护':'price-books'};
  ...
  <div className="shell"><header><span>业务管理 <i>/</i><b>{Object.keys(pages).find(label=>pages[label]===activePage)}</b></span><div className="identity-bar">{session&&<><select aria-label="当前企业" disabled={busy} value={tenant ?? ''} onChange={e=>void switchTenant(e.target.value)}><option value="" disabled>请选择企业</option>{session.memberships.map(m=><option key={m.id} value={m.id}>{m.name}</option>)}</select><button disabled={busy} onClick={()=>void logout()}>退出登录</button></>}</div></header><main>
    <div className="heading"><div><div className="eyebrow">SALES & CUSTOMER INTELLIGENCE</div><h1>{activePage==='analytics'?'让每个判断，都有真实依据':activePage==='service'?'让每次维修，都有完整履历':activePage==='finance'?'让每笔往来，都有清晰依据':activePage==='delivery'?'让每次交付，都有据可查':activePage==='crm'?'从一次成交，到长期合作':activePage==='inventory'?'从每批到货，到清晰库存':activePage==='quotes'?'把算力，组装成你的理想形态':activePage==='contracts'||activePage==='orders'?'让每项约定，都有清晰来源':'从每个部件，到清晰版本'}<span>。</span></h1><p>{activePage==='analytics'?'从签约、交付到资金与库存，以可追溯的经营事实连接每个环节。':activePage==='service'?'设备代管、备件消耗与服务收费分开登记，保留安装和维修历史。':activePage==='finance'?'应收应付、实收实付与核销分别登记，保留完整依据与历史。':activePage==='delivery'?'逐台测试、分批发货，连接验收、退货与设备履历。':activePage==='crm'?'独立客户档案，连接联系人、项目与内部责任分工。':'商品、技术组成与商业价格分开维护，来源与未知状态清晰可查。'}</p></div><span className="date">工作台 / {new Date().toLocaleDateString('zh-CN',{year:'numeric',month:'2-digit',day:'2-digit'}).replaceAll('/','.')}</span></div>
    {message&&<div className="error" role="alert">{message}</div>}{notice&&<p className="success" role="status">{notice}</p>}
    {!session?<section className="panel login-panel"><h2>{authLoading?'正在读取身份…':'进入客户工作空间'}</h2><p>使用企业身份登录后，选择企业访问客户档案。</p><a className="primary button-link" href="/api/v1/auth/login">使用企业身份登录</a><button onClick={()=>void loadSession()}>刷新登录状态</button></section>:!tenant?<section className="panel login-panel"><h2>请选择企业</h2><p>{session.memberships.length?'在右上方选择你要进入的企业。':'尚未加入企业，请联系管理员。'}</p></section>:activePage==='analytics'?<Analytics key={tenant+session.context_id+'analytics'} context={crmRequests.capture()} onContextError={handleError} onOpen={t=>{setAnalyticsTarget(t);if(t.kind==='customer'){setActivePage('crm');void openCustomer(t.id);}else setActivePage(({contract:'contracts',order:'orders',shipment:'delivery',finance:'finance',inventory:'inventory',service:'service'} as const)[t.kind]);}}/>:activePage==='service'?<Service key={tenant+session.context_id+'service'} context={crmRequests.capture()} onContextError={handleError} preferredDeviceId={serviceDevice} preferredWorkId={analyticsTarget?.kind==='service'?analyticsTarget.id:''}/>:activePage==='finance'?<Finance preferredSource={analyticsTarget?.kind==='finance'?analyticsTarget.id:''} key={tenant+session.context_id+'finance'} context={crmRequests.capture()} onContextError={handleError}/>:activePage==='delivery'?<Delivery preferredShipment={analyticsTarget?.kind==='shipment'?analyticsTarget.id:''} key={tenant+session.context_id+'delivery'} context={crmRequests.capture()} onContextError={handleError}/>:activePage==='assembly'||activePage==='devices'?<Assembly key={tenant+session.context_id+activePage} context={crmRequests.capture()} members={members} deviceOnly={activePage==='devices'} onOpenService={id=>{setServiceDevice(id);setActivePage('service');}} onContextError={handleError}/>:activePage==='inventory'?<Inventory preferredLayer={analyticsTarget?.kind==='inventory'?analyticsTarget.id:''} key={tenant+session.context_id+'inventory'} context={crmRequests.capture()} members={members} onContextError={handleError}/>:activePage==='quotes'?<QuoteStudio publicationEnabled key={tenant+session.context_id+'quotes'} context={crmRequests.capture()} canWrite={session.memberships.find(m=>m.id===tenant)?.role!=='viewer'} canDiscount={session.memberships.find(m=>m.id===tenant)?.role==='admin'} disabled={busy} onContextError={handleError}/>:(activePage==='contracts'||activePage==='orders')?<Contracts preferredId={analyticsTarget&&['contract','order'].includes(analyticsTarget.kind)?analyticsTarget.id:''} key={tenant+session.context_id+activePage} context={crmRequests.capture()} members={members} canWrite={canWrite} canSign={session.memberships.find(m=>m.id===tenant)?.role==='admin'} ordersOnly={activePage==='orders'} onOpenQuotes={()=>setActivePage('publication')} onContextError={handleError}/>:activePage==='publication'?<PublicationDesk key={tenant+session!.context_id+activePage}  context={crmRequests.capture()} canApprove={session?.memberships.find(m=>m.id===tenant)?.role==='admin'} canWrite={canWrite} disabled={busy} onContextError={handleError}/>:activePage!=='crm'?<Catalog key={tenant+session.context_id+activePage} page={activePage} context={crmRequests.capture()} canWrite={session.memberships.find(m=>m.id===tenant)?.role==='admin'} disabled={busy} onContextError={handleError}/>:<>
      ...
      <div className="analysis-filters"><span>客户管理</span><span className="muted">报价、合同签约登记及销售订单已启用；设备实物与库存尚未启用</span></div>
