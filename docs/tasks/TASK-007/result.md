...
- 已验收基线：`59f55c813aba62ec8f18dc5d5e7cc649b277aeca`。
- 独立验收交接：`87e5d80f7127f4ceb2e9d06f082473f3cb9698ab`。
- 实现、迁移及验证：`1f19d3dc3c8dd967218e1083b95b0e4f75d4a61a`。
- 输入控件样式细化及构建：`2fb0e796267711c36a6ba42ae19724cabb6c2127`。
- 后续交付文档提交只登记上述commit、差异与复现；最终完整HEAD写入包外的REVIEW_MANIFEST.md，避免本文件自引用commit的不可能循环。

样式细化只统一既有控件样式；最终构建通过，真实浏览器截图为细化前版本，具体边界见browser-acceptance末尾。文件字节未改的PDF测试夹具明确作为binary管理，保留其xref必需填充；不是为掩盖源码空白错误。

## 独立审查 R1/R2 修复与 C1 补充（2026-09-11）

状态保持 `review_ready`；本轮基准 `732089f2e812920c2e20b863cf9749b4dd6dad3e`。独立报告原样归档到 `docs/reviews/TASK-007-732089f-review.md`，SHA256 `fc607a71ccad6119daf7258a11b9069f252adabd4b96998ac27e82ca93cf703e`。此前结果、限制和截图保留，不自行认定独立复核通过。

R1：原签后非补充附件的一刀切保护错误覆盖未进入冻结集的pending。现在同一合同事务下，仅允许删除这种未引用pending；关联仍拒绝，已冻结证明仍不可删。沿用幂等重放和当前权限/企业检查，不提升冻结workspace版本。真实PG覆盖逻辑删除及24h清理只移除B、A字节/签约/订单不变、权限撤销和跨企业拒绝、删除与签约竞争。前端同步按钮及解释，实际组件测试覆盖删除生命周期。

R2：原头尾标记可接受伪造PDF/JPEG。新增独立有界解码进程，PDF逐页实际渲染，PNG/JPEG校验再完整加载；拒绝畸形、截断、过大页数/像素。不改变上传原字节；新关联/新签约重检未冻结旧文件，历史已签文件保持不变。仅为本修复增加固定pypdfium2/Pillow/psutil版本及锁文件，未升级其他依赖；版本与来源见parser-versions.json，限制及非AV边界见ADR-012。无数据库迁移或API契约变更。

实际验证：

- R1红测1失败→1通过；R2红测3失败（真实接口接受伪造格式）→实际解码定向5通过。`r2-green.txt`曾2失败/3通过，为Darwin不支持RLIMIT_DATA的实现兼容诊断，不冒称绿色；改为跨平台父进程RSS监测后通过，原日志保留。
- 完整后端164 passed、7 warnings、107.98秒，真实独立PG17.11/Keycloak26.7.3，覆盖身份/租户/CRM/目录/报价发布修订/Worker及迁移。随后新增2个资源限额测试独立通过（40 deselected），没有伪称重新完整收集166项。合同40项全过加后续2项限额均有实际日志。
- Node19通过；真实React组件合同6、发布5、草稿3通过（HTTP替身单独标注）；类型检查/构建通过，OpenAPI及生成类型字节一致。
- 浏览器真实可信HTTPS/IdP、上传拒绝→有效PDF关联→签约→旧pending删除→重载v2及订单查询通过。会话中途失效重新登录后重试成功，保留前后证据。C1桌面1440×900/移动390×844最终源码截图4张均已目视核对，未重采样或覆盖原截图。
