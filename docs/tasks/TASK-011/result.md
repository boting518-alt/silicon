...
status: review_ready。修复前基准 `680c071c96309137788671c160c1df5235fa59a6`；本节为追加历史，不改上方原执行结果。

原样归档 [独立审查报告](../../reviews/TASK-011-680c071-review.md)，附件逐字节相同。报告为 changes_requested；本轮执行者完成修复不等于独立复核通过。

原因：原额度占用正确保留了负向调整前的计划金额，但把恢复误减当成普通正向增额，且无受控更正入口。现在增加 `correct-adjustment`，只完整撤销同计划原负向调整，恢复金额由原不可变事实决定。唯一约束、相同事务锁和 expected_version 防重复/并发恢复；幂等作用域含计划及原调整，重放仍检查权限与关系。

更正额进入有效金额和对账，不增加来源占用；原计划和普通正调整仍占来源额度，不能通过真实减额另建重复债权。原调整、更正与销售退货/退款事实各自保留。实际退款后更正只恢复未结额，不伪造退款撤销；原退货 ID 防重不释放。前端在历史原调整旁提供明确更正入口，显示原因、依据及关联。仅支持完整恢复，不支持客户端指定部分恢复或恢复正向调整。

迁移 `0014_finance_corrections` 增加复合外键、唯一关系、负额校验、RLS 和不可变记录。不覆盖0013资金事实；无更正时可降级0013，有更正时拒绝降级并回滚。旧幂等计划响应缺少 corrections 时兼容为空；恢复事实不写回旧命令。详情、金额及恢复口径见 [ADR-016](../../architecture/adr/ADR-016.md)。

新增真实 PG/API 场景包括应收/应付1000→800→1000且来源始终1000、更正后核销守恒、并发仅一次、跨计划/跨企业、撤权后重放拒绝、不能恢复正额或伪造恢复金额、真实退货退款后更正及防重、0013旧事实升级与非空更正降级拒绝。原14项资金回归继续覆盖，扩展其中退货场景。

实际页面使用原300合同，完成300→误减200→恢复300→刷新保留，未调整合同额度；真实 PG 的1000示例与浏览器证据不混称。新组件测试验证实际按钮、丢失响应重试、仅一次更正和原记录显示；HTTP替身与真实业务浏览器分开。截图沿原设计，新增历史关联为有意变化。

红测为2项新场景失败、14项原测试通过；当时星号导入导致重复收集原测试，之后改为明确导入，未把重复测试计入新增覆盖。首轮绿测2项通过，扩展定向20项通过。完整回归首次242项通过、1项IdP启动错误；Keycloak8080端口冲突的证据保留，清理独立浏览器栈后串行重跑。最终结果及提交登记追加如下。
