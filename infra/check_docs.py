...
assert config['current_task'] == 'TASK-005'
...
assert f"status: {config['status']}" in (root/'docs/tasks/TASK-005/task.md').read_text()
assert '当前只执行已明确分配的 TASK-005' in (root/'AGENTS.md').read_text()
