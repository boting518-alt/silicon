"""Export committed TASK-009 R1 repair source; refuse dirty trees or unsafe materials."""
...
base='c08079e0e7ce4c79c86728817a7841b4c9f214e8'
handoff=base
...
with tempfile.TemporaryDirectory(prefix='silicon-task009-review-') as temporary:
    ...
    (evidence/'INDEX.md').write_text('# Evidence index\n\n- [TASK-009 result](../source/docs/tasks/TASK-009/result.md)\n- [Validation and exact commands](../source/docs/tasks/TASK-009/evidence/r1/validation.json)\n- [Browser procedure and limitations](../source/docs/tasks/TASK-009/evidence/r1/browser-acceptance.md)\n- [Screenshots](../source/docs/tasks/TASK-009/evidence/r1/screenshots.json)\n- [Task](../source/docs/tasks/TASK-009/task.md)\n- [ADR-014](../source/docs/architecture/adr/ADR-014.md)\n- [Original design baseline](../source/docs/design/demo-baseline.md)\n\nFull raw logs and screenshots are under source/docs/tasks/TASK-009/evidence/r1; original evidence remains alongside. Prior reports and evidence are retained under source without duplicate copies.\n')
    (root/'REVIEW_MANIFEST.md').write_text(f'''# SILICON TASK-009 R1 incremental review package
...
''')
    ...
    with tempfile.TemporaryDirectory(prefix='silicon-task009-patch-') as d:
        ...
        for f in (old/'docs/tasks/TASK-009').rglob('*'):
            if not f.is_file():continue
            target=source/f.relative_to(old)
            if f.name=='result.md':assert target.read_bytes().startswith(f.read_bytes())
            else:assert target.read_bytes()==f.read_bytes(),str(f)
    ...
    for name in ['apps/api/migrations/versions/0011_assembly_corrections.py','apps/api/tests/test_assembly.py','apps/web/src/Assembly.tsx','docs/tasks/TASK-009/task.md','docs/tasks/TASK-009/evidence/r1/validation.json','docs/tasks/TASK-009/evidence/r1/screenshots.json']:
        assert (source/name).is_file(),name
    (root/'PACKAGE_CHECKS.json').write_text(json.dumps({'final':head,'tracked_files':len(tree),'commit_blobs_exact':True,'binary_patch_replay_exact':True,'prior_reports_and_task007_history_unchanged':True,'task008_historical_evidence_and_result_prefix_unchanged':True,'symlinks':0,'secret_scan':'no high-confidence matches; fictional fixtures retained','task009_original_evidence_unchanged_and_result_append_only':True,'independent_acceptance':False},indent=2)+'\n')
