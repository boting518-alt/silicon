"""Export committed TASK-010 R1 incremental review source; refuse dirty trees or unsafe materials."""
...
base='9da23ab4aef6fdc766a68379a694c65bef3f8f4b'
handoff=base
...
with tempfile.TemporaryDirectory(prefix='silicon-task010-review-') as temporary:
    ...
    for name in ['AGENTS.md','docs/tasks/backlog.md','docs/tasks/TASK-010/task.md']:
    ...
    (evidence/'INDEX.md').write_text('# Evidence index\n\n- [TASK-010 result](../source/docs/tasks/TASK-010/result.md)\n- [Validation and exact commands](../source/docs/tasks/TASK-010/evidence/r1/validation.json)\n- [Browser procedure and limitations](../source/docs/tasks/TASK-010/evidence/r1/browser-acceptance.md)\n- [Screenshots](../source/docs/tasks/TASK-010/evidence/r1/screenshots.json)\n- [Task](../source/docs/tasks/TASK-010/task.md)\n- [ADR-015](../source/docs/architecture/adr/ADR-015.md)\n- [Original design baseline](../source/docs/design/demo-baseline.md)\n\nNew raw logs and screenshots are under source/docs/tasks/TASK-010/evidence/r1. Original evidence remains unchanged. Prior reports and evidence are retained under source without duplicate copies.\n')
    (root/'REVIEW_MANIFEST.md').write_text(f'''# SILICON TASK-010 R1 incremental review package

Status: review_ready, not accepted. TASK-011 not started.

- Review baseline (not accepted): {base}
- Fix start: {handoff}
- Original accepted TASK-009: 17088be77dd1472ce11287f7b6ae88cfcb0bf721
- Original TASK-010 handoff: c6d5bc3a2d4e1a2569ed18f3e6816f45503610d6
- Original TASK-010 implementation: 663725a3887c2cb15222182d661d9f9642a5aa0f
- Implementation: {implementation}
- Final HEAD: {head}
- Worktree: clean at export; branch main. Ancestry verified.

source/ is ALL tracked files archived from final HEAD. changes/ has binary full-index patches, file status, stats and logs for base→final, handoff→implementation, implementation→final. review-base contains prior AGENTS/backlog and explicitly marks TASK010 task absent. SHA256SUMS excludes itself. PACKAGE_CHECKS validates blobs, patch replay, historical evidence and full unpacked checksums. No uncommitted content is included.
...
PG/Keycloak scripts verify pinned download checksums. Existing destinations are not overwritten. Tests create random-port temporary PG clusters and migration/runtime roles, then stop and remove only those clusters. OIDC generates a repository-owned CA and verifies hostnames. Browser stack uses its own PG/IdP/API/file roots with fictional sales contract and inventory fixtures, three prior-task completions preseeded; no tests or delivery transactions preseeded. SIGTERM/Ctrl-C cleans owned processes and logs BROWSER_STACK_CLEANED. Do not use resident README bootstrap to run tests.
...
Open https://localhost:5173 and use fictional alice/Fictional-alice-17!; choose enterprise A. B is viewer with no business data. Follow source/docs/tasks/TASK-010/evidence/r1/browser-acceptance.md (relative to package root) for the actual R1 flow: save a test for DEL-0; open base device detail and authorized history; use `.venv/bin/python infra/device_permission_fixture.py revoke` in a second terminal at source/; reopen detail without refreshing permission metadata to exercise real 403; verify base survives and history disappears. Navigate to assembly and search devices, refresh at mobile viewport; run `restore` to restore the fixture's prior delivery.read grant and refresh/reopen; switch to B and log out. This fixture refuses non-disposable databases, accepts no URL and only changes the fictional admin role in the running test stack. No system trust changes.

`/tests/device-permissions.html` (3 tests), `/tests/assembly-component.html` (2), and `/tests/delivery-component.html` (2) run actual React with HTTP substitutes; they are not PG or browser business acceptance. Original full delivery flow is preserved in historical evidence, not claimed as repeated in this R1-only run.

## Evidence and limitations

Read validation.json and result.md for exact test outcomes and intermediate failures. Local passing commands do not imply remote CI or independent review passed. TASK008 historical download-artifact limit remains; no new file flow. No Docker, other-browser, production deployment or real supplier/tax-policy verification. Manual tests and physical delivery enabled; no hardware certification, financial payment, normal dismantling or labor overhead enabled. Evidence references are archive registration numbers, not uploaded reports; third-party devices without existing supported completion are not supported. Historical absolute paths in raw logs are evidence only.
...
''')
    ...
    with tempfile.TemporaryDirectory(prefix='silicon-task010-patch-') as d:
        ...
        for f in (old/'docs/tasks/TASK-009').rglob('*'):
            if f.is_file() and f.name!='task.md':assert f.read_bytes()==(source/f.relative_to(old)).read_bytes(),str(f)
        for f in (old/'docs/tasks/TASK-009/evidence').rglob('*'):
            if f.is_file():assert f.read_bytes()==(source/f.relative_to(old)).read_bytes(),str(f)
        historical=old/'docs/tasks/TASK-009/result.md'
        assert (source/'docs/tasks/TASK-009/result.md').read_bytes().startswith(historical.read_bytes())
        for f in (old/'docs/tasks/TASK-010').rglob('*'):
            if not f.is_file():continue
            actual=(source/f.relative_to(old)).read_bytes()
            if f.name=='result.md':assert actual.startswith(f.read_bytes()),str(f)
            else:assert actual==f.read_bytes(),str(f)
    ...
    for name in ['apps/api/migrations/versions/0012_delivery.py','apps/api/tests/test_delivery.py','apps/web/src/Delivery.tsx','apps/web/src/Assembly.tsx','apps/web/tests/device-permissions.jsx','docs/reviews/TASK-010-9da23ab-review.md','docs/tasks/TASK-010/task.md','docs/tasks/TASK-010/evidence/r1/validation.json','docs/tasks/TASK-010/evidence/r1/screenshots.json']:
        assert (source/name).is_file(),name
    (root/'PACKAGE_CHECKS.json').write_text(json.dumps({'final':head,'tracked_files':len(tree),'commit_blobs_exact':True,'binary_patch_replay_exact':True,'prior_reports_and_task009_history_unchanged':True,'task009_historical_evidence_and_result_prefix_unchanged':True,'symlinks':0,'secret_scan':'no high-confidence matches; fictional fixtures retained','task010_original_evidence_unchanged_result_append_only':True,'independent_acceptance':False},indent=2)+'\n')
...
with tempfile.TemporaryDirectory(prefix='silicon-task010-unzip-') as d:
    root=Path(d)
