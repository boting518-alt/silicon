"""Export committed TASK-010 review source; refuse dirty trees or unsafe materials."""
...
base='17088be77dd1472ce11287f7b6ae88cfcb0bf721'
handoff='c6d5bc3a2d4e1a2569ed18f3e6816f45503610d6'
...
with tempfile.TemporaryDirectory(prefix='silicon-task010-review-') as temporary:
    ...
    (evidence/'INDEX.md').write_text('# Evidence index\n\n- [TASK-010 result](../source/docs/tasks/TASK-010/result.md)\n- [Validation and exact commands](../source/docs/tasks/TASK-010/evidence/validation.json)\n- [Browser procedure and limitations](../source/docs/tasks/TASK-010/browser-acceptance.md)\n- [Screenshots](../source/docs/tasks/TASK-010/evidence/screenshots.json)\n- [Task](../source/docs/tasks/TASK-010/task.md)\n- [ADR-015](../source/docs/architecture/adr/ADR-015.md)\n- [Original design baseline](../source/docs/design/demo-baseline.md)\n\nFull raw logs and screenshots are under source/docs/tasks/TASK-010/evidence. Prior reports and evidence are retained under source without duplicate copies.\n')
    (root/'REVIEW_MANIFEST.md').write_text(f'''# SILICON TASK-010 independent review package
...
Open https://localhost:5173 and use fictional alice/Fictional-alice-17!; choose enterprise A. B is viewer with no business data. Follow browser-acceptance.md for actual test→split shipment→partial acceptance→actual return→retest→reship→refresh flow and two viewports. `/tests/delivery-component.html` runs actual React with HTTP substitutes and is separately labelled, never substitutes for real PG/browser evidence.
...
''')
    ...
    for name in ['apps/api/migrations/versions/0012_delivery.py','apps/api/tests/test_delivery.py','apps/web/src/Delivery.tsx','docs/tasks/TASK-010/task.md','docs/tasks/TASK-010/evidence/validation.json','docs/tasks/TASK-010/evidence/screenshots.json']:
        assert (source/name).is_file(),name
    (root/'PACKAGE_CHECKS.json').write_text(json.dumps({'final':head,'tracked_files':len(tree),'commit_blobs_exact':True,'binary_patch_replay_exact':True,'prior_reports_and_task009_history_unchanged':True,'task009_historical_evidence_and_result_prefix_unchanged':True,'symlinks':0,'secret_scan':'no high-confidence matches; fictional fixtures retained','independent_acceptance':False},indent=2)+'\n')
