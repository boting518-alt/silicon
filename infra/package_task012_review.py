"""Export committed TASK-012 review source; refuse dirty trees or unsafe materials."""
...
base='6d89a1023d06b795686bfbc1504da7dde0baf099'
...
with tempfile.TemporaryDirectory(prefix='silicon-task012-review-') as temporary:
    ...
    for name in ['AGENTS.md','docs/tasks/backlog.md','docs/tasks/TASK-012/task.md']:
...
PG/Keycloak scripts verify pinned download checksums. Existing destinations are not overwritten. Tests create random-port temporary PG clusters and migration/runtime roles, then stop and remove only those clusters. OIDC generates a repository-owned CA and verifies hostnames. Browser stack uses its own PG/IdP/API/file roots with fictional sales contract and inventory fixtures, three prior-task completions, one shipment and explicit fictional opening spares preseeded through existing APIs; no service works, swaps, RMA, service charges, finance plans, cash or allocations preseeded. SIGTERM/Ctrl-C cleans owned processes and logs BROWSER_STACK_CLEANED. Do not use resident README bootstrap to run tests.
...
Read docs/tasks/TASK-012/evidence/validation.json, browser-acceptance.md and result.md for exact outcomes and intermediate failures. Local tests do not imply remote CI or independent review passed. Service remains manual registration: no vendor, bank, remote-control or production integrations. CNY only; integer pieces; manual warranty/compatibility and explicit ownership/cost basis. Reports are archive identifiers, no new file upload path. Historical TASK008 download limitations remain. Not every API branch was replayed in a browser. Test IdP session expiry was observed and reauthentication used, never bypassed. See implementation.md for precise migration downgrade limits and fixture instructions. Raw logs may contain historical machine paths, not runtime requirements.
...
''')
    ...
    with tempfile.TemporaryDirectory(prefix='silicon-task012-patch-') as d:
        ...
        for f in (old/'docs/tasks/TASK-011').rglob('*'):
            if f.is_file() and f.name!='task.md':assert f.read_bytes()==(source/f.relative_to(old)).read_bytes(),str(f)
        for f in (old/'docs/tasks/TASK-011/evidence').rglob('*'):
            if f.is_file():assert f.read_bytes()==(source/f.relative_to(old)).read_bytes(),str(f)
        historical=old/'docs/tasks/TASK-011/result.md'
    ...
    for name in ['apps/api/migrations/versions/0015_service.py','apps/api/tests/test_service.py','apps/web/src/Service.tsx','docs/tasks/TASK-012/task.md','docs/tasks/TASK-012/evidence/validation.json','docs/tasks/TASK-012/evidence/screenshots.json']:
        assert (source/name).is_file(),name
    (root/'PACKAGE_CHECKS.json').write_text(json.dumps({'final':head,'tracked_files':len(tree),'commit_blobs_exact':True,'binary_patch_replay_exact':True,'prior_reports_and_task011_history_unchanged':True,'task011_historical_evidence_and_result_prefix_unchanged':True,'symlinks':0,'secret_scan':'no high-confidence matches; fictional fixtures retained','independent_acceptance':False},indent=2)+'\n')
...
with tempfile.TemporaryDirectory(prefix='silicon-task012-unzip-') as d:
    root=Path(d)
